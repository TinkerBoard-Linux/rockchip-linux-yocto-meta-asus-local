...
# Switch sound device
echo "Config default sound output device: $1" > /dev/kmsg
...
#echo "Switch running actvice stream to another sound output device" > /dev/kmsg
...
sudo -u weston PULSE_RUNTIME_PATH=/run/user/1000/pulse pactl move-sink-input `echo $line | cut -d'#' -f2` $1
