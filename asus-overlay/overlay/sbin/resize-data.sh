...
DATA_DEVICE="/dev/mmc${MMC}p$PART_ENTRY_NUMBER"
echo "resize: DATA_DEVICE=$DATA_DEVICE" > /dev/kmsg

PARTITION_SIZE=$(cat /sys/class/block/mmc${MMC}/mmc${MMC}p$PART_ENTRY_NUMBER/size)
echo "resize:PARTITION_SIZE=$PARTITION_SIZE" > /dev/kmsg

if [ "$PARTITION_SIZE" -le "4194304" ]; then
...
mount /dev/mmc${MMC}p$PART_ENTRY_NUMBER /data
